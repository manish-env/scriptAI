...
export default defineEventHandler(async (event) => {
  ...
  if (event.method === 'PATCH') {
    const { scene_id, image_key } = await readBody(event)
    if (!scene_id || !image_key) throw createError({ statusCode: 400, message: 'scene_id and image_key required' })
    if (env.DB) await env.DB.prepare('UPDATE scenes SET image_key = ? WHERE id = ? AND session_id = ?').bind(image_key, scene_id, session_id).run()
    return { ok: true }
  }
})
